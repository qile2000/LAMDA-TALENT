...
import numpy as np
from TALENT.model.lib.tabnet import sparsemax
...
class GBN(torch.nn.Module):
    ...
    def forward(self, x):
        chunks = x.chunk(int(np.ceil(x.shape[0] / self.virtual_batch_size)), 0) # 拆分
...
class TabNetEncoder(torch.nn.Module):
    ...
    def forward(self, x, prior=None):
        ...
        M_loss = 0
        # dim = self.n_d + self.n_a
        # pca = PCA(n_components=dim)
        # # print("shape:", self.initial_splitter(x).device)
        
        # att_cpu = self.initial_splitter(x).detach().cpu().numpy()
        # att_pca = pca.fit_transform(att_cpu)
        # att = torch.from_numpy(att_pca).to(x.device)[:, self.n_d :]
        att = self.initial_splitter(x)[:, self.n_d :] #这里可以考虑怎么选择特征
        steps_output = []
        for step in range(self.n_steps):
            ...
            # update attention
            att = out[:, self.n_d :] #这里也是
            # att_pca = pca.fit_transform(out.detach().cpu().numpy())
            # att = torch.from_numpy(att_pca).to(x.device)[:, self.n_d :]
