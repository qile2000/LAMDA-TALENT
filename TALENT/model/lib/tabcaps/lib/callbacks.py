...
class CallbackContainer:
    ...
    def on_train_end(self, logs=None):
        ...
        return self.trainer.network
    
    def resturn_res(self):
        return self.trainer.best_cost, self.trainer.lowest
...
class EarlyStopping(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        lowest = logs.get('loss')
        if lowest < self.lowest:
            self.lowest = lowest
        ...
        if max_improved or min_improved:
            ...
            if self.trainer.log:
                # save_dict = {'params': self.trainer.params,
                #              'decode': self.trainer.decode,
    ...
    def on_train_end(self, logs=None):
        ...
        self.trainer.lowest = self.lowest
...
class History(Callback):
    ...
    def on_train_begin(self, logs=None):
        # self.history = {"loss": [], "reconstruct_metric": [], 'reconstruct_mse': []}
        self.history = {"loss": []}
        self.history.update({"lr": []})
        self.history.update({name: [] for name in self.trainer._metrics_names})
        # print("_metric_names:", self.trainer._metrics_names)
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # print("epoch_metrics:", self.epoch_metrics)
